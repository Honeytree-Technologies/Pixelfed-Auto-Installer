...
########################################################################
#                                                                      #
#               Pixelfed Installation and Hardening Script             #
#                                                                      #
...
#####                                                              #####
#####   This will install Pixelfed on fresh server                 #####
#####                                                              #####
#####  Installing on an operating Pixelfed server will wipe data   #####
#####                                                              #####
########################################################################
startup_warning
...
while true; do
  read -p "Enter valid Mail driver name: " mail_driver
  if [ -n "${mail_driver}" ]; then
    break
  else
    echo "Mail driver cannot be empty. Please enter mail driver."
  fi
done


while true; do
  read -p "Enter valid Mail host: " mail_host
  if [ -n "${mail_host}" ]; then
    break
  else
    echo "Mail host cannot be empty. Please enter mail host."
  fi
done
...
while true; do
  read -p "Enter Mail address: " mail_addr
  if [ -n "${mail_addr}" ]; then

    break
  else
    echo "Mail address cannot be empty. Please enter mail address."
  fi
done
...
while true; do
  ...
  if [ -n "${mail_password}" ]; then
    ...
    echo "Mail password cannot be empty. Please enter mail password."
  fi
done

while true; do
  read -p "Enter Mail encrption: " mail_encryption
  if [ -n "${mail_encryption}" ]; then
    break
  else
    echo "Mail encrption cannot be empty. Please enter  mail encryption."
  fi
done


read -p "Enter the DB USER NAME (Default: pixelfed): " db_username
...
temp_db="pixel_$(random_string 8)"
...
if nginx -v &>/dev/null; then
  ...
  rm /etc/nginx/sites-available/pixelfed
  rm /etc/nginx/sites-enabled/pixelfed
else
  sudo apt-get update
...
touch /etc/nginx/sites-available/pixelfed
...
sudo ln -s /etc/nginx/sites-available/pixelfed /etc/nginx/sites-enabled/pixelfed
